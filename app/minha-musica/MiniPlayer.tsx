      ...
      {fullOpen && (
        <div className="fixed inset-0 z-50 text-white overflow-hidden" style={{ background: FUNDO, transition: "background-color 500ms ease" }}>
          ...
          >
            ...
            {/* Rodapé fixo: compartilhar + o caminho pra frente.
                O link é /rede/{orderId}, NÃO o /m/{slug} do player do
                pedido: aquele mostra as fotos do cliente, e o slug é a
                credencial delas. A música aqui é de outra pessoa — mandar
                o link dela pro WhatsApp reabriria o vazamento que a gente
                acabou de fechar. A página da Rede mostra capa, letra e
                player, e nenhuma foto.
                Só aparece em música DA REDE: a própria já tem o link
                completo (com fotos) na aba Pedidos. */}
            <div
              ...
              {naRede && (
                <div className="flex items-center justify-center gap-3">
                  <button
                    onClick={async () => {
                      try {
                        await navigator.clipboard.writeText(`${window.location.origin}/rede/${track.id}`)
                  ...
                  >
                    <span className="w-10 h-10 rounded-full bg-white/8 border border-white/10 flex items-center justify-center group-hover:bg-white/14 group-active:scale-95 transition-all">
                      {copiado ? (
                  ...
                  <a
                    href={`https://wa.me/?text=${encodeURIComponent(`Ouve essa música que achei na Fiz Música: "${track.title}" — ${typeof window !== "undefined" ? window.location.origin : ""}/rede/${track.id}`)}`}
