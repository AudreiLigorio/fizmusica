...
export default function SyncPaymentButton({ orderId }: { orderId: string }) {
  ...
  const [result, setResult]   = useState<{ ok: boolean; message: string; mpStatus?: string } | null>(null)
  ...
  return (
    <div className="mt-4 pt-4 border-t border-white/10">
      ...
      >
        {loading ? (
          <><span className="w-3.5 h-3.5 border-2 border-white/40 border-t-white rounded-full animate-spin" /> Consultando MP…</>
        ) : "🔄 Verificar pagamento no MP"}
      </button>
      {result && (
        <p className={`mt-2 text-xs text-center ${result.updated ? "text-green-400" : result.mpStatus === "pending" ? "text-yellow-400" : "text-gray-500"}`}>
          {result.message}
        </p>
      )}
    </div>
