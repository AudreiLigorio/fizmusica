...
- **Cron:** `vercel.json` agenda `/api/cron/recovery` 1x/dia (limite do plano Hobby) — faz repescagem multi-toque, expurgo de fotos/lead não pago, e expurgo de fotos de pedido pago quando o link expira.
...
- **Fotos/lead de pedido NÃO pago:** expurgo automático (cron diário) — fotos em 7 dias (obrigatório LGPD, imagem de terceiro sem compra), lead em 60 dias (interesse legítimo, prazo declarado). Parametrizável em `/admin/operacao` (`purge_settings`), com relatório (`purge_log`).
